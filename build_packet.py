#!/usr/bin/env python3
"""
BUILD_PACKET  v2.2  ·  Hypersourced client packet renderer  ·  2026-08-05 03:15
...
# Placeholder tokens the payload writes on purpose, and what they resolve to in a
# rendered packet. RECRUITER and [CALLBACK_NUMBER] are correct INSIDE the swarm
# payload -- section 9 STEP 5 puts them there so the operator can find-and-replace
# downstream. They are wrong in a client-facing document: the one place we show a
# client our actual craft is the sample copy, and an unresolved token there reads
# as a template we forgot to finish.
TOKENS = {
    "RECRUITER": "recruiter_name",
    "[CALLBACK_NUMBER]": "recruiter_phone",
    "[RECRUITER_NAME]": "recruiter_name",
    "[PHONE]": "recruiter_phone",
}


def resolve_tokens(text, m):
    """Swap payload placeholders for the real values before anything renders."""
    out = str(text)
...
def render_outreach(m, c=None):
    ...
    for chan, when, subj, text in samples:
        subj, text = resolve_tokens(subj, m), resolve_tokens(text, m)
...
def nextcue(m):
    """Wayfinding under the closing box: the roster starts on the next page.

    Section 5 ends with a call to action, which reads like the end of the
    document. Section 6 is the largest thing in the packet and a reader who
    stops at the button never sees it. One quiet line and a chevron, rendered
    only when next_cue is present in the content file.
    """
    cue = m.get("next_cue")
    if not cue:
        return ""
    return ('<div class="nextcue"><p class="nc-t">' + E(cue) +
            '</p><span class="nc-a"></span></div>')
...
def main():
    ...
    # NO PLACEHOLDER MAY REACH A CLIENT. Cheap check, loud failure -- a token in a
    # sent PDF cannot be repaired.
    leaked = [t for t in TOKENS if t in doc]
    if leaked:
        print(f"  PLACEHOLDER LEFT IN THE PACKET: {', '.join(leaked)}")
        print("  Set recruiter_name / recruiter_phone in the content file. "
              "Do not send this PDF.")
