#!/usr/bin/env python3
"""
BUILD_PACKET  v1.1  ·  Hypersourced client packet renderer  ·  2026-08-04 22:55
...
def load_candidates(xlsx_path, aliases, elite_band=None, featured=10):
    ...
    return dict(total=total, eliminated=eliminated, scored=n, dropped=dropped,
                ...
                featured=keep[:featured], featured_n=min(featured, n))
...
def dimension(heading, ceiling, body, tiers, extra=""):
    """One scoring dimension as a self-contained block."""
    return (f'<div class="dim"><p class="dim-h">{E(heading)}</p>'
...
def render_results(c, m):
    ...
    head = (f'<div class="fbanner">'
            f'<p class="big">{c["total"]:,}</p>'
            f'<p class="cap">{E(d["banner_caption"])}</p>'
            f'<p class="sub">{ruled_out:,} ruled out before scoring &middot; '
            f'{c["scored"]:,} scored and ranked</p></div>')

    if not bands:
        return (f'<h3 class="bignote">{E(d["headline"])}</h3>\n<p>{E(d["lede"])}</p>\n{head}'
                f'\n<p style="margin-top:26px">{E(d["closing"])}</p>')

    top_name, _, top_count = bands[0]
    hero = (f'<div class="hero"><div class="hn">{top_count:,}</div><div>'
            f'<p class="hl">{E(top_name)}</p>'
            f'<p class="hd">{E(desc.get(top_name, ""))}</p></div></div>')

    strip = ['<div class="strip">']
    for name, _cut, count in bands[1:]:
        strip.append(f'<div class="c"><p class="sn">{count:,}</p>'
...
def li_button(r, li_b64):
    """The LinkedIn mark, hyperlinked to that candidate's own profile.

    Returns empty string when either the mark or the URL is missing, so a row with
    no resolved profile renders as the rank disc alone rather than a broken image
    or a link to nowhere. Every row in the Nabla pull has a URL; that will not
    always be true, and a dead LinkedIn button in a PDF cannot be repaired.
    """
    if not li_b64 or not r.get("url", "").startswith("http"):
        return ""
    return (f'<a class="cand-li" href="{E(r["url"])}" '
            f'title="LinkedIn profile" aria-label="LinkedIn profile"></a>')
...
def render_featured(c, m, li_b64=""):
    ...
    for r in c["featured"]:
        ...
        out.append(
            ...
            '      </div>\n'
            f'      <span class="cand-rank">{li_button(r, li_b64)}'
            f'<i>{r["rank"]}</i></span>\n'
            '    </div>\n'
...
def render_method(m):
    ...
    return "\n".join([
        ...
        dimension(dna_h.strip(), dna_c.strip() or "up to 200 points",
                  f'<p>{E(d["dna_body"])}</p>\n<p>{E(d["dna_axis"])}</p>',
                  tiers_block(d["dna_tiers"])),
        dimension(role_h.strip(), role_c.strip() or "up to 150 points",
                  f'<p>{E(d["role_body"])}</p>',
                  tiers_block(d["role_tiers"]),
                  f'<p class="aside">{E(d["role_note"])}</p>'),
        '<div class="pagebreak-here"></div>',
        f'<h3>{E(d["rest_heading"])}</h3>',
        f'<p>{E(d["rest_body"])}</p>',
...
def render_outreach(m):
    ...
    for chan, when, subj, text in m["samples"]:
...
def build(content, cands, template, logo_b64, meta_html="", li_b64=""):
    ...
    if report:
        ...
        S.append(section("02", "Your top ten", "Ideal candidates",
                         render_featured(cands, m, li_b64), brk=True))
        S.append(section("03", "Method", "Two pillars", render_method(m), brk=True))
        S.append(section("04", "Outreach", "How we reach them", render_outreach(m), brk=True))
        S.append(sec05)
        ...
        S.append(section("03", "Method", "How every candidate will be scored", render_method(m), brk=True))
        S.append(section("04", "Outreach", "How we reach them", render_outreach(m), brk=True))
        S.append(sec05)
...
<table class="status" role="presentation"><tr>
  ...
  Prepared for {E(m['client'])} &middot; {E(m['role'])} &middot; {E(m['packet_date'])}<br>
  Packet template v1.1 &middot; prototype, not for client use</p>
</footer>'''
    ...
    if li_b64:
        mark_css = ("\n<style>\n"
                    f'.cand-li,.li-note .li-mark i{{background-image:'
...
def render_pdf(html_path, pdf_path):
    ...
    WHY CHROMIUM AND NOTHING ELSE:
      ...
      sheet, which measured at 0.76-0.79x across every element tested. Body text
      the stylesheet sets at 9.6pt was reaching clients at 7.5pt, and section
      headings at 12.1pt instead of 16pt. Nobody could see it, because the whole
      document shrank together and looked deliberate.

      Chromium prints what the stylesheet says. The document got longer when we
      switched; it did not get bigger. It was always this long -- it was being
      photographed at three-quarter scale.

      Chromium is also a current browser, so the PDF and the screen agree. That
      closes the entire class of bug that produced the funnel, the masthead and
      the mobile-stylesheet problems.

    preferCSSPageSize honours the @page rule in the print stylesheet, so paper
    size and margins are declared in one place -- the CSS -- and not split
    between the CSS and this function.
    """
